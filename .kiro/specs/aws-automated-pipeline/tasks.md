...
- [ ] 1. Extract TokenProvider interface and add shared retry utility
  - [ ] 1.1 Create `TokenProvider` interface in `com.extractor.auth` with a single `getAccessToken()` method
  ...
  - [ ] 1.2 Create `RetryExecutor` utility in `com.extractor.util` with configurable max retries and exponential backoff
  ...
  - [ ] 1.4 Enhance `PageInfo` record to include `lastModifiedDateTime` field
...
- [ ] 2. Implement CredentialLoader and GraphCredentialsAuthModule
  - [ ] 2.1 Create `CredentialLoader` in `com.extractor.config` that reads SSM Parameter Store SecureString parameters
  ...
  - [ ] 2.3 Create `GraphCredentialsAuthModule` in `com.extractor.auth` implementing `TokenProvider`
...
- [ ] 6. Implement GoogleAuthModule and NotebookLmUploader
  - [ ] 6.1 Create `GoogleAuthModule` in `com.extractor.auth` that loads service account credentials from JSON string
    - Read service account JSON from `PipelineCredentials`
    - Return `GoogleCredentials` for API calls
    - _Requirements: 5.2_
  - [ ] 6.2 Create `NotebookLmUploader` in `com.extractor.notebooklm` with `uploadPdf` method
    - Use `RetryExecutor` for upload retries
    - Accept PDF bytes and filename
    - _Requirements: 5.1, 5.3_

- [ ] 7. Implement MetricsPublisher and PipelineReporter
  - [ ] 7.1 Create `MetricsPublisher` in `com.extractor.metrics` that emits CloudWatch metrics
...
- [ ] 12. Create AWS CDK infrastructure
  - [ ] 12.1 Create CDK app in `infra/` directory using Python
    - Define stack with: EventBridge scheduled rule, Lambda function (container image), S3 bucket, DynamoDB table, SSM parameters, CloudWatch alarm on failures
  ...
  - [ ] 12.2 Create Dockerfile for Lambda container image
    - Base on `public.ecr.aws/lambda/java:25` or equivalent JDK 25 image
    - Copy fat JAR, set handler to `com.extractor.pipeline.PipelineHandler::handleRequest`
    - _Requirements: 9.1_
